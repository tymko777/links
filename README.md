...
```bash
git clone https://github.com/tymko777/links.git
cd links
...
The main window supports these shortcuts on both Wayland and X11:

| Shortcut | Action |
| --- | --- |
| `Ctrl+K` | Focus and select the search field |
| `Ctrl+N` | Create a new card in the selected folder |
| `Ctrl+Shift+N` | Create a new folder |
| `Ctrl+I` | Import a JSON or YAML configuration |
| `Ctrl+Shift+E` | Export the current configuration |

Enable **Edit** mode to reveal editing controls. In that mode, cards and
actions can be reordered by drag-and-drop, and the duplicate buttons create
independent copies with fresh IDs.

Click the star on any card to add it to Favorites. The star filter in the
header shows only favorite cards in the selected folder. Every saved change
keeps the previous valid JSON document at
`~/.config/links/links.json.bak`; use **Restore last backup** from the menu
when you need to undo the last saved state.

Select **All cards** in the sidebar to search and browse every folder at once.
Cards show their folder name in this mode, while edits and drag-and-drop still
operate on the original folder.

## Headless CLI

The optional `links-cli` command inspects the same configuration without
opening GTK. It is useful for shell scripts, launchers, and quick checks:

```bash
links-cli --list
links-cli --search github
links-cli --config-home /tmp/links-test --list
```
